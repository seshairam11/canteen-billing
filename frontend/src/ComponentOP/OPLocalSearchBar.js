...
export const OPLocalSearchBar = ({ ctl_Attribute, rerender }) => {
  ...
  function handleOnMouseDown(value) {
    ctl_Attribute.inputvalue = value;
    ctl_Attribute.tooltip.isvalidation = true;
  ...
  function handleOnChange(e) {
    ...
    fltData.current = ctl_Attribute.dropdownlist.filter(item => String(item.m_value.menu).toLowerCase().includes(String(e.target.value).toLowerCase()))
    l_setRerender(!l_rerender);
  }
    ...
    <>
      <div className="">
        <div className="select2-results">
          ...
          {isShowList && (
            <ul className='pos-absolute wth-77 table-view' style={{ backgroundColor: "white" }}>
              {fltData.current.map(item => {
                return (
                  <li
                    key={item.m_key}
                    className='pd-10 popup-toggle list-search'
                    onMouseDown={() => handleOnMouseDown(item.m_value.menu)}
                    value={item.m_value.menu}
                  >
