...
export const OPServerOrderPayment = () => {
    const [startInti, setStartIniti] = useState(false);
    ...
    function fnFinishSelectedButton() {
        ...
        const newContent = tbl_menu.current.tableData.map(item => ({
            ...
            table_value: [
                {
                    t_key: 0,
                    t_value: item.l_table_value[0].t_value
                },
                {
                    t_key: 1,
                    t_value: item.l_table_value[1].t_value
                },
                {
                    t_key: 2,
                    t_value: item.l_table_value[2].t_value
                },
                {
                    t_key: 3,
                    t_value: "Waiting"
        ...
        socketRef.current.emit('updateCasherBilling', docId, newContent);
        document.body.style = "";
        setSideBar(false);
        setStartIniti(!startInti)
    }
                ...
                const clonedData = structuredClone(ctlBench.current.groupbtndata[index].btn_values).map(item => {
                    ...
                    return {
                        ...
                        l_table_value: [
                            ...
                            { t_key: 3, t_value: item.table_value[3].t_value },
                        ],
                        table_value: [
                            { t_key: 0, t_value: item.table_value[0].t_value },
                            { t_key: 1, t_value: item.table_value[1].t_value },
                            { t_key: 2, t_value: filteredMenu[0].price },
                            { t_key: 3, t_value: Number(item.table_value[1].t_value) * filteredMenu[0].price }
                        ]
                    }
    ...
    useEffect(() => {
        ...
        socketRef.current.emit('joinCasherBilling', getAppStoreData._id);
                                            ...
                                            <div className="card">
                                                <div className="card-body" id='card-body'>
                                                    <div className='border-bottom mb-3 pb-3'>
                                                        ...
                                                        <p>Your order is ready. Please proceed to payment</p>
                                                    </div>
                                                    <OPGroupButton
                            ...
                            <>
                                ...
                                <div className='offcanvas-body'>
                                    ...
                                    <div className="d-flex justify-content-end">
                                        <h6 >
                                            <span style={{ marginRight: "2rem" }}>GST@ 18%  &nbsp; :&nbsp; {(price_Total.current / 18).toFixed(2)}</span>
                                        </h6>
                                    </div>
                                    <div className="d-flex justify-content-end">
                                        <h3 >
                                            <span style={{ marginLeft: "2rem" }}>Total &nbsp; : {(price_Total.current / 18 + price_Total.current).toFixed(2)}</span>
                                        </h3>
                                    </div>
