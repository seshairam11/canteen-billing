...
export const OPServerOrderProcessing = () => {
    ...
    function fnSaveSelectedButton() {
        ...
        const newContent = tbl_menu.current.tableData.map(item => ({
            ...
            table_value: [
                ...
                {
                    t_key: 3,
                    t_value: item.table_value[3].t_value === "Processing" || item.table_value[3].t_value === "New" ? "Processing" : item.table_value[3].t_value === "Waiting" ? "Waiting" : "Completed"
                },
            ]
        }));
        ctlBench.current.groupbtndata.splice(removableIndex, 1)
        socketRef.current.emit('updateServerOrderProcessing', docId, newContent);
    ...
    function handleGroupButtonClick(groupBtnName, index) {
        ...
        console.log(ctlBench.current.groupbtndata[index]);
    ...
    useEffect(() => {
        ...
        socketRef.current.emit('joinServerOrderProcessing', "mgr orderup", getAppStoreData.collageid);
                                            ...
                                            <div className="card">
                                                <div className="card-body" id='card-body'>
                                                    <div className='border-bottom mb-3 pb-3'>
                                                        ...
                                                        <p>Wait until the order is processed!</p>
                                                    </div>
                                                    <OPGroupButton
