...
export const OPEmployeeLogin = () => {
    ...
    function fnLoginResult() {
        ...
        const _values = responseData.value
        if (responseData.isAuth) {
            dispatchappStore(
                setlogininfo({
                    ...
                    usertype: _values.employeeType.toLowerCase(),
                    mailID: _values.mailID,
                    phone: _values.phone,
                }),
            );
            navigate(`/${responseData.value.employeeType.toLowerCase()}/my-profile`);
        } else {
